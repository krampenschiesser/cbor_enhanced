...
pub fn derive_protocol(item: TokenStream) -> TokenStream {
    ...
    generate_serialize(&parsed).into()
}

fn generate_serialize(input: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl crate::Serialize for #identifier {
...
fn get_serialized_fields(data: &Data, identifier: &Ident, id_checker: &mut IdChecker) -> Vec<proc_macro2::TokenStream> {
    match data {
        ...
        Data::Enum(my_enum) => {
            let variants: Vec<_> = my_enum.variants.iter().map(|v| {
                ...
                if no_fields {
                    if let Some(attribute) = found {
                        ...
                        v.span().unwrap().error(format!("No #[id(?)] attribute given")).emit();
                        unreachable!()
                    }
                } else {
            ...
            let variants: Vec<_> = variants.iter().map(|(literal, identifier, field_token_stream, field_names)| {
...
fn get_field_names(fields: &Fields) -> Option<Either<Vec<Ident>, Vec<Index>>> {
    ...
    let ret = if all_named {
        Either::A(fields.iter().map(|f| {
            f.ident.clone().expect("Should be named")
        }).collect())
    } else {
        Either::B(fields.iter().enumerate().map(|(field_id, f)| {
            Index {
                span: f.span().clone(),
...
fn serialize_fields(fields: &Fields, is_enum: bool, id_checker: &mut IdChecker) -> Vec<proc_macro2::TokenStream> {
    fields.iter().enumerate().map(|(field_id, f)| {
        let default_attribute = f.attrs.iter().find(|a| a.path.is_ident("default"));
        let default_attribute = default_attribute.map(|a| syn::parse2::<Group>(a.tokens.clone()).unwrap());
        ...
        if let Some(attribute) = found {
            ...
            f.span().unwrap().error(format!("No #[id(?)] attribute given")).emit();
            unreachable!()
        }
    }).map(|(id_literal, identifier, default_attribute)| {
        let parsed: syn::LitInt = syn::parse(quote! {#id_literal}.into()).unwrap();
        let id = parsed.base10_parse::<usize>().expect("Could not parse number");
        if !id_checker.check_add_id(id) {
            id_literal.span().unwrap().error(format!("Duplicate id {}", id)).emit();
...
fn get_field_amount(data: &Data) -> Option<usize> {
    match data {
        ...
        Data::Union(my_union) => {
            Some(my_union.fields.named.len())
        }
    }
}
...
fn get_reserved_ids(attrs: &[Attribute]) -> Vec<usize> {
    let found = attrs.iter().find(|a| a.path.is_ident("reserved"))
        .map(|a| {
            let group: Group = syn::parse2(a.tokens.clone()).unwrap();

            let vec: Vec<usize> = group.stream().into_iter().filter_map(|tree| {
                match tree {
                    TokenTree::Literal(lit) => {
                        let parsed: syn::LitInt = syn::parse(quote! {#lit}.into()).unwrap();
                        Some(parsed.base10_parse::<usize>().expect("Could not parse number"))
                    }
                    _ => None
                }
            }).collect();
...
impl IdChecker {
    ...
    pub fn check_add_id_literal(&mut self, lit: &Literal) -> bool {
        ...
        if ok {
            ...
            lit.span().unwrap().error(format!("Duplicate id {}", id)).emit();
            panic!("Do not reuse id's");
        }
    }
