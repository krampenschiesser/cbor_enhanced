...
impl<'de> Deserializer {
    pub fn take_ip_address(&self, data: &'de [u8]) -> Result<(IpAddr, Remaining<'de>), CborError> {
        ...
        if slice.len() == 4 {
            ...
            Err(CborError::InvalidArrayLength { expected: &[4, 16], got: slice.len() })
        }
    }
    pub fn take_ip_address_and_mask(&self, data: &'de [u8]) -> Result<((IpAddr, u8), Remaining<'de>), CborError> {
        ...
        if length.unwrap_or(0) != 1 {
            return Err(CborError::InvalidArrayLength { expected: &[1], got: length.unwrap_or(0) });
        }
    ...
    pub fn take_mac_address(&self, data: &'de [u8]) -> Result<([u8; 6], Remaining<'de>), CborError> {
        let remaining = self.expect_tag(data, IanaTag::NetworkAddress)?;
        let (slice, remaining) = self.take_bytes(remaining, true)?;
        ...
        if slice.len() == 6 {
            ...
            Err(CborError::InvalidArrayLength { expected: &[6], got: slice.len() })
        }
    }
}
